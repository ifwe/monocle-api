...
var generateError = function(message, reason) {
    var error = new Error(message);
    // @ts-ignore adding dynamic property deliberately
    error.reason = reason;
    return error;
}
    ...
    for (var key in object) {
        ...
        if (isArray) {
            ...
            for (var arrayKey in value) {
                var arrayValue = validateAndCastParam.call(this, key, value[arrayKey], schemaProperty.items);
...
function getErrorCode(type) {
    ...
    var code = validationErrors[type].code;
