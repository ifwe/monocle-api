...
Request.prototype.setUrl = function(url) {
    ...
    this.deleteQuery('props');

    this._etag = null;
};

Request.prototype.getValidationErrors = function() {
...
Request.prototype.setQueries = function(query) {
...
Request.prototype.deleteQuery = function(query) {
...
Request.prototype.validateAndCastParams = function(object) {
    return this.validateAndCast(this.getParams());
}

Request.prototype.validateAndCastQueries = function(object) {
    return this.validateAndCast(this.getQueries());
 ...
 * Query parameters that are defined as type "integer", "number", "boolean" or "string" are converted accordingly.
 * Pushes errors to validationErrors array if errors are gound
 *
 * @param object - object to validate
 */
Request.prototype.validateAndCast = function(object) {
    ...
    for (var key in object) {
        ...
        if (!schemaProperty) {
            throw new Error("Unexpected query param " + key + " not found in schema.");
        }
        ...
        if (schemaProperty.type.indexOf('integer') !== -1) {
            value = parseInt(value);

            if (isNaN(value)) {
                this._validationErrors.push(typeError(key));
                return;
            }
        }

        if (schemaProperty.type.indexOf('float') !== -1) {
        ...
        if (schemaProperty.type.indexOf('boolean') !== -1) {
            switch(value) {
                case 'true' :
                case true :
                    value = true;
        ...
        if (schemaProperty.type.indexOf('string') !== -1) {
            value = value.toString();
        }

        if (schemaProperty.enum && schemaProperty.enum.indexOf(value) === -1) {
            this._validationErrors.push(typeError(key, 'enum'));
            continue;
        }
        ...
        //If the key is a parameter of this url set the value
    ...
    var validate = jsen(this._schema);
    var valid = validate(object);

    if (!valid) {
        this._validationErrors = validate.errors;
        return;
    }
}
