...
Router.prototype.handle = function(request, connection) {
    ...
    if (!!~[Request.METHOD_PATCH, Request.METHOD_POST, Request.METHOD_PUT].indexOf(method)) {
        var validate = jsen(_.extend({additionalProperties: false}, route.schema));
        ...
        if (resource) {
            ...
            if (!valid) {
                ...
                if (validate.errors[0].key && validationErrors[validate.errors[0].key]) {
                    code = validationErrors[validate.errors[0].key].code;
                    message = validationErrors[validate.errors[0].key].message;
                }
