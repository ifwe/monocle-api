...
Router.prototype.route = function (patternConfig, schema, handlers) {
    let newRoute = Route.createRoute(patternConfig, schema, handlers);
...
Router.prototype.alias = function (patternConfig, aliasResolver) {
    let newRoute = Route.createAlias(patternConfig, aliasResolver);
...
Router.prototype.documentAll = function () {
    ...
    if (null === documentAllCache) {
        documentAllCache = Promise.map(this._routeObjects, (route) => route.getDocumentation())
          .then((routes) => {
              return routes.sort((a, b) => {
                  if (a.alias && !b.alias) return 1;
                  if (b.alias && !a.alias) return -1;
                  if ((a.alias && b.alias) || (!a.alias && !b.alias)) {
                      let nameA = a.schema && a.schema.title || a.pattern;
                      let nameB = b.schema && b.schema.title || b.pattern;
...
Router.prototype.handle = function (request, connection) {
    ...
    if (request.isOptions()) {
        return Promise.resolve(routeObject.getDocumentation());
    }
