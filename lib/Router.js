...
Router.prototype.route = function(patternConfig, schema, handlers) {
    ...
    for (var queryParam in query) {
        ...
        var schemaProperty = filter.property(queryParam);
        ...
        //If null are allowed and the value is null than don't make the conversions or checks below
        if (!(query[queryParam] === null && schemaProperty.type.indexOf('null') !== -1)) {
            if (schemaProperty.type.indexOf('integer') !== -1) {
                query[queryParam] = parseInt(query[queryParam]);
            }


            if (schemaProperty.type.indexOf('number') !== -1) {
                query[queryParam] = parseFloat(query[queryParam]);
            }


            if (isNaN(query[queryParam]) || schemaProperty.enum && schemaProperty.enum.indexOf(query[queryParam]) === -1) {
...
Router.prototype.handle = function(request, connection) {
    ...
    for (var i = 0, len = route.keys.length; i < len; i++) {
        ...
        if (props.hasOwnProperty(paramName)) {
            //If null are allowed and the value is null than don't make the conversions or checks below
            if (!(paramValue === null && props[paramName].type.indexOf('null') !== -1)) {
                if (props[paramName].type.indexOf('integer') !== -1) {
                    paramValue = parseInt(paramValue);
                }


                if (props[paramName].type.indexOf('number') !== -1) {
    ...
    if (_.isFunction(route.handlers[method])) {
        ...
        if (typeof resource !== 'undefined' && [Request.METHOD_PATCH, Request.METHOD_PUT].indexOf(method) !== -1) {
            ...
            handlers = getFunctionHandlers(request, route, keys);
        } else if (props.length) {
            handlers = getFunctionHandlers(request, route, props);
        } else {
