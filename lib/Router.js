...
Router.prototype.handle = function(request, connection) {
    ...
    if (requestRouter.hasPropertyErrors()) {
        ...
        return requestRouter.error(422, 'Properties do not validate with schema');
    }

    if (!route.handlers.hasOwnProperty(method)) {
        emit.call(this, 'api:error', requestRouter, null, timeStart);
        return requestRouter.error(404, "No " + method + " handler for " + resourceId);
    }
    ...
    if (!handlers.length) {
        emit.call(this, 'api:error', requestRouter, null, timeStart);
        return requestRouter.error(404, "No handlers available for resource " + resourceId);
    }
